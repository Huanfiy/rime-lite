...
- **中英切换**：左 Shift 点按或 Ctrl+Space 翻转中英（Shift+字母仍打大写）。
- **固定短语置顶**：`custom_phrase.txt` 缩写码词条固定靠前（`gpio` → GPIO、`zkb` → 占空比）。
- **超高频英文 / 命令**：`en_priority.txt` 在中文输入下也整词首选（`gs`、`cd`、`ls`）。
...
│   ├── pinyin.dict.yaml
│   ├── custom_phrase.txt
│   ├── en_priority.txt      # 超高频英文 / 命令，中文下也首选
│   ├── lua/ascii_shift.lua  # 左 Shift / Ctrl+Space 翻转中英（D-28）
