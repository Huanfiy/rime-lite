#!/usr/bin/env python3
"""rime-lite AI 候选 daemon（D-17 / D-18 / D-21 / D-22 / D-23）。
...
DEFAULTS = {
    ...
    "context_commits": 30,        # 会话上下文保留的上屏条数（逐词上屏时一条≈一个词，取大让 context_chars 成为实际上限）
...
# 候补口径单点（ai-daemon.md §8，D-23 / D-29）：严格不超过 200 字；保留领域消歧、
# 机械候选抗锚定与已选前缀约束；三槽定职（转写 / 备选转写 / 表情或短续写），
# 用「固定输出」而非「可」——temperature=0 下可选项等于永不输出。
SYSTEM_PROMPT = (
    "你是面向嵌入式开发者的拼音输入法。结合上文与已选前缀转写拼音，可纠正拼音误写、展开首字母缩写；"
    "同音优先电子、半导体、编程术语，英文缩写、芯片型号、API名保留规范写法。"
    "本地机械转换仅供切分，勿照抄。"
    "固定输出3行：第1行最可能的转写；第2行另一种合理转写（同音、纠错或缩写展开）；"
    "第3行表情意图时给匹配emoji，否则在转写后按上文续写≤8字。无序号解释，不重复已选前缀。"
)
