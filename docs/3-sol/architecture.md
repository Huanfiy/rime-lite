...
│   ├── cn_dicts/                # 中文基础词库项目内副本（D-4，D-12 更名）
│   │   ├── 8105.dict.yaml       # 字表
│   │   └── base.dict.yaml       # 基础词库
│   └── en_dicts/
...
- `pinyin.dict.yaml`：入口文件，`import_tables: [cn_dicts/8105, cn_dicts/base]`；除迁移自现用工程的 26 个大写字母词条（Shift + 字母输入、大写字母参与造句）外不含词条。新增词库时只改此文件的 import 列表。
