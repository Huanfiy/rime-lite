...
| D-14 | userdb 晋升标准（阶段 3） | 分析源为 `docs/1-ref/rime-ice/` 两份导出（D-2）；按（词, 拼音）合并两份 c 值，门槛 c_total ≥ 3；排除 c ≤ 0、单字、已收录词（base / 8105 / embedded / mydict / custom_phrase / A–Z 词条）；人工剔除组句残留与错词；晋升词条以 `# ========== userdb 晋升 (日期) ==========` 分区追加至目标词库，统一权重 100 |
