...
| 编号   | 决策                | 说明                                                              |
| ---- | ----------------- | --------------------------------------------------------------- |
| D-26 | 中英切换：左 Shift 点按翻转，超时 500ms | 倒回 D-24 前 `Shift_L: commit_code` 语义（翻转 `ascii_mode`；组词中 `ClearNonConfirmed` + `Commit`），改由 Lua 实现（librime 1.10.0 超时硬编码，Lua 可调；现行 500ms 与引擎默认一致）。仅在左 Shift 维持中按下其它键才打断；松开超时不切。右 Shift / Caps 不切中英。`ascii_composer` 的 `Shift_*` / `Caps_Lock` 仍 `noop`（Caps 本机 `caps:ctrl_modifier`，不恢复原 `clear`）。返回 `kNoop`。热路径仅 keycode 分支，受 D-19 预算约束。 |
...
│   └── lua/                     # 热路径 Lua（D-18 / D-26；红线预算制见 D-19）
│       ├── ascii_shift.lua      # 左 Shift 点按切中英（D-26，500ms）
...
另保留：方案选单（switcher）、ASCII 切换（ascii_composer：`Caps_Lock` / `Shift_*` 为 noop；左 Shift 点按由 D-26 Lua 处理）、标点映射（punctuator，供方案以 `import_preset: default` 引用）、通用 recognizer patterns（email / url / underscore）、基础 key_binder（光标移动、`-` / `=` 翻页、中英标点切换）。
...
engine:
  processors:
    - lua_processor@*ascii_shift     # 左 Shift 点按切中英（D-26，500ms）
    - ascii_composer
    - recognizer
    - key_binder
